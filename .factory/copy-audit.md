# Landing-page copy audit

Audited 2026-09-02. Counts treat hyphenated terms and numeric ranges as one
word. No sentence exceeds 22 words. No banned word appears.

| Copy | Words | Type |
| --- | ---: | --- |
| Bridge Crew | 2 | wordmark |
| Demo | 1 | navigation |
| How it works | 3 | navigation |
| Privacy | 1 | navigation |
| 4–8 players · 12 minutes | 4 | fact |
| Run a browser-tab spaceship repair game | 6 | h1 |
| For teachers and group hosts sharing one display while players control four station panels. | 14 | sentence |
| Try it with sample data | 5 | action |
| Opens a repair already in progress. | 6 | sentence |
| Free to play | 3 | fact |
| No accounts or chat | 4 | fact |
| Keyboard and touch controls | 4 | fact |
| Start here | 2 | label |
| Open a room on this browser | 6 | h2 |
| Project the host tab. | 4 | sentence |
| Open each station in another tab on the same browser profile. | 11 | sentence |
| Create a room | 3 | action |
| Room code | 2 | label |
| Join room | 2 | action |
| Live bridge preview | 3 | label |
| One fault needs four stations | 5 | h2 |
| Players say what they see. | 5 | sentence |
| Every repair needs their answers in order. | 7 | sentence |
| Signals must scan the fault before the repair starts. | 9 | sentence |
| How it works | 3 | label |
| Get the crew playing in three steps | 7 | h2 |
| Create a room | 3 | h3 |
| Project the bridge and read the five-character room code aloud. | 10 | sentence |
| Open four stations | 3 | h3 |
| Assign Helm, Power, Signals, and Engineering. | 6 | sentence |
| Extra players share controls. | 4 | sentence |
| Repair together | 2 | h3 |
| Call out each clue, align the ship, route power, and enter the repair code. | 14 | sentence |
| Privacy and limits | 3 | label |
| A game, not a student account | 6 | h2 |
| No names, chat, cameras, or recordings. | 6 | sentence |
| Room state stays in this browser and expires after 20 minutes. | 11 | sentence |
| This static v1 connects tabs in one browser profile. | 9 | sentence |
| It does not yet connect separate devices. | 7 | sentence |
| Bridge Crew is a free, 12-minute cooperative browser game. | 9 | footer sentence |
| Original generated scene · v1.0.0 | 5 | provenance |

## Terminology

| Concept | One term |
| --- | --- |
| Shared play state | room |
| Large projected view | bridge |
| Player control area | station |
| Damage event | fault |
| Ship health | integrity |
| Accessibility option | assist mode |
| Isolated sample | demo |
